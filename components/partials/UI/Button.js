...
export function Button({ children, hideDecoration = false, effect = false }) {
  return <div className=" relative group duration-200 w-fit overflow-hidden ">
    <div className={`${effect ? 'btn-effect' : 'bg-black'} absolute group-hover:scale-125 duration-200  -left-2.5 -top-2.5  w-[24px] h-[24px] rounded-full`}></div>
    ...
    <div className={`absolute -right-3 group-hover:scale-125 duration-200 -bottom-3  w-[34px] h-[34px] rounded-full
      ${effect ? 'btn-effect' : 'bg-black'}  ${hideDecoration ? 'hidden md:block' : ''}`}></div>
  </div>
}